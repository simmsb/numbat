...
impl Approximation {
    pub fn approximate(&self, signal: &Option<Signal>, p: Precision) -> BigInt {
        ...
        match self {
            ...
            Component(c, integer_component) => component(signal, c, *integer_component, p),
        }
    }
}
...
fn component(
    ...
    c: &Computable,
    integer_component: bool,
    p: Precision,
) -> BigInt {
    // let msd = c.iter_msd_stop(p).unwrap_or(p);
    // let msd = c.iter_msd();
    let bits = -p;

    // dbg!(msd, bits, p, msd - bits);

    // msd > 0, msd - (p + msd): -p
    // msd < 0, msd - p: msd - p
    // println!("Custom backtrace: {}", std::backtrace::Backtrace::force_capture());

    let x = c.approx_signal(signal, p);
    let (s, x) = x.into_parts();

    // dbg!(&x);

    let v = match (integer_component, bits < 0) {
        (true, false) => (x >> bits) << bits,
        (true, true) => x,
        (false, false) => x & (BigUint::one() - (BigUint::one() << bits)),
        (false, true) => BigUint::zero(),
    };

    // dbg!(bits);
    // dbg!(s);
    BigInt::from_biguint(s, v)
}
...
fn ratio(r: &Rational, p: Precision) -> BigInt {
    if p >= 0 {
        // p >= 0, n * 2 ** -p / d
        //
        // higher p -> smaller out
        scale(r.shifted_big_integer(0), -p)
    } else {
        // p < 0, (n/d).s_b_i(-p) -> n * (2 ** -p) / d
        //
        // lowe p -> larger out
