...
impl std::str::FromStr for Rational {
    ...
    fn from_str(s: &str) -> Result<Self, Problem> {
        ...
        if let Some((n, d)) = s.split_once('/') {
        ...
        } else if let Some((i, d_m_exp)) = s.split_once('.') {
            ...
            let (whole, d) = if let Some((d, exp)) = d_m_exp.split_once(&['e', 'E']) {
                let mut exp_s = true;
                let exp = if let Some(s) = exp.strip_prefix('-') {
                    exp_s = false;
                    s
                } else {
                    exp
                };
                let exp = exp.parse().ok().ok_or(Problem::BadDecimal)?;
                let exp = TEN.pow(exp);
                (
                    Self {
                        sign: whole.sign,
                        numerator: if exp_s {
                            whole.numerator * exp.clone()
                        } else {
                            whole.numerator
                        },
                        denominator: if !exp_s {
                            whole.denominator * exp
                        } else {
                            whole.denominator
                        },
                    },
            ...
            let (exp_num, exp_denom, s) = if let Some((s, exp)) = s.split_once(&['e', 'E']) {
                let mut exp_s = true;
                let exp = if let Some(s) = exp.strip_prefix('-') {
                    exp_s = false;
                    s
                ...
                let exp = TEN.pow(exp);
                (
                    if exp_s { exp.clone() } else { BigUint::one() },
                    if !exp_s { exp } else { BigUint::one() },
                    s,
                )
            } else {
                (BigUint::one(), BigUint::one(), s)
            };
            ...
            Ok(Self {
                sign,
                numerator: numerator * exp_num,
                denominator: exp_denom,
            })
        }
    }
}
