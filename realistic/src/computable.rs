...
impl Computable {
    ...
    pub fn trunc(self) -> Self {
        Self {
            internal: Box::new(Approximation::Component(self, true)),
            cache: RefCell::new(Cache::Invalid),
            signal: None,
        }
    }
    ...
    pub fn fract(self) -> Self {
        Self {
            internal: Box::new(Approximation::Component(self, false)),
            cache: RefCell::new(Cache::Invalid),
            signal: None,
        }
    }
    ...
    pub fn known_msd(&self) -> Precision {
    ...
    pub fn msd(&self, p: Precision) -> Option<Precision> {
        ...
        if try_once {
            ...
            let improved = if let Cache::Valid((prec, prev)) = &*self.cache.borrow() {
                // prev != &appr
                true
            } else {
                true
            ...
            if appr.magnitude() < &BigUint::one() && improved {
                return None;
            }
        }
    ...
    pub fn iter_msd_stop(&self, p: Precision) -> Option<Precision> {
        let mut prec = 0;
    ...
    pub fn iter_msd(&self) -> Precision {
