...
impl Real {
    ...
    fn abs(self) -> Result<Real, Problem> {
        (self.clone() * self).sqrt()
    }

    pub fn rem_euclid(self, other: Self) -> Result<Self, Problem> {
        let r = (self % other.clone())?;
        if r.best_sign() == Sign::Minus {
            Ok(r + other.abs()?)
        } else {
            Ok(r)
        }
...
impl Rem for Real {
    ...
    fn rem(self, other: Self) -> Result<Self, Problem> {
        if other.definitely_zero() {
            return Err(Problem::DivideByZero);
        }
        if self.definitely_zero() {
        ...
        // trunc(a * (1 / b)) don't log this, if the truncated value is < 0
        // it'll loop forever as it can never provide something above 0
        let trnc = a.clone().multiply(b.clone().inverse()).trunc();

        let subtr = trnc.multiply(b);
