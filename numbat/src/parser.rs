...
//! primary         ::=   boolean | string | hex_number | oct_number | bin_number | number | identifier ( struct_expr ? ) | "(" expression ")" | let_expr
//! struct_expr     ::=   "{" ( identifier ":" type_annotation "," )* ( identifier ":" expression "," ? ) ? "}"
//! let_expr        ::=   "let" identifier "=" expression "of" expression
//!
...
pub enum ParseErrorKind {
    ...
    #[error("Expected one of: number, identifier, parenthesized expression, struct construction, let expression")]
    ExpectedPrimary,
    ...
    #[error("Expected 'of' after expression in let expression")]
    ExpectedOfInLetExpression,
}
    ...
    fn primary(&mut self) -> Result<Expression> {
        ...
        if let Some(num) = self.match_exact(TokenKind::Number) {
            ...
            if self.match_exact(TokenKind::Of).is_none() {
                return Err(ParseError {
                    kind: ParseErrorKind::ExpectedOfInLetExpression,
                    span: self.peek().span,
                });
            }
...
mod tests {
    ...
    fn accumulate_errors() {
        ...
        Errors encountered:
        Expected one of: number, identifier, parenthesized expression, struct construction, let expression - ParseError { kind: ExpectedPrimary, span: Span { start: SourceCodePositition { byte: 4, line: 1, position: 5 }, end: SourceCodePositition { byte: 5, line: 1, position: 6 }, code_source_id: 0 } }
        "###);
        ...
        Errors encountered:
        Expected one of: number, identifier, parenthesized expression, struct construction, let expression - ParseError { kind: ExpectedPrimary, span: Span { start: SourceCodePositition { byte: 50, line: 3, position: 24 }, end: SourceCodePositition { byte: 51, line: 3, position: 25 }, code_source_id: 0 } }
        "###);
        ...
        Successfully parsed:
        Errors encountered:
        Expected one of: number, identifier, parenthesized expression, struct construction, let expression - ParseError { kind: ExpectedPrimary, span: Span { start: SourceCodePositition { byte: 40, line: 3, position: 17 }, end: SourceCodePositition { byte: 41, line: 3, position: 18 }, code_source_id: 0 } }
        "###);
        ...
        Expected one of: number, identifier, parenthesized expression, struct construction, let expression - ParseError { kind: ExpectedPrimary, span: Span { start: SourceCodePositition { byte: 36, line: 3, position: 17 }, end: SourceCodePositition { byte: 40, line: 3, position: 21 }, code_source_id: 0 } }
        Expected one of: number, identifier, parenthesized expression, struct construction, let expression - ParseError { kind: ExpectedPrimary, span: Span { start: SourceCodePositition { byte: 63, line: 4, position: 17 }, end: SourceCodePositition { byte: 67, line: 4, position: 21 }, code_source_id: 0 } }
        "###);
