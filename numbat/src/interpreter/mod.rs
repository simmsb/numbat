...
pub enum RuntimeError {
    ...
    #[error("Mathmatical error: {0}")]
    MathError(realistic::Problem),
}

impl From<realistic::Problem> for RuntimeError {
    fn from(value: realistic::Problem) -> Self {
        RuntimeError::MathError(value)
    }
}

impl From<realistic::Problem> for Box<RuntimeError> {
    fn from(value: realistic::Problem) -> Self {
        Box::new(RuntimeError::MathError(value))
    }
}
...
mod tests {
    ...
    fn assert_evaluates_to_scalar(input: &str, expected: f64) {
        assert_evaluates_to(input, Quantity::from_scalar(expected.try_into().unwrap()))
    }
    ...
    fn arithmetic_with_units() {
        ...
        assert_evaluates_to(
            "2 meter + 3 meter",
            Quantity::from_scalar((2.0 + 3.0).try_into().unwrap()) * Quantity::from_unit(Unit::meter()),
        );
        ...
        assert_evaluates_to(
            "dimension Pixel
             ...
             unit pixel : Pixel
             2 * pixel",
            Quantity::from_scalar(2.0.try_into().unwrap())
        ...
        assert_evaluates_to(
            ...
            Quantity::from_scalar(5.0.try_into().unwrap())
